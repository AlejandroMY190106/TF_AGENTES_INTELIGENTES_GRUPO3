"""
tc_pipeline/ml-training/model_trainer_LogReg.py
─────────────────────────────────────────────────
Módulo de Entrenamiento y Serialización del Clasificador Regresión Logística.
...
Responsabilidad Arquitectónica:
    ...
    ``LogisticRegression`` con regularización regular, y
    serializa los artefactos resultantes (modelo joblib + LabelEncoder joblib).
"""
...
def train(
    ...
    """Entrena el clasificador LogisticRegression y serializa los artefactos."""
    if cfg is None:
        cfg = MLConfig()
    ...
    model_path = cfg.logreg_model_artifact_path
    encoder_path = cfg.logreg_encoder_artifact_path

    joblib.dump(model, str(model_path), compress=3)
    logger.info("Modelo LogisticRegression guardado en: '%s'", model_path)

    joblib.dump(encoder, str(encoder_path), compress=3)
    logger.info("LabelEncoder de LogisticRegression guardado en: '%s'", encoder_path)
