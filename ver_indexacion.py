...
# 1. Nos conectamos a la base de datos local
...
try:
    ...
    for i in range(len(resultado["ids"])):
        ...
        print(f"Tipo de Expediente: {resultado['metadatas'][i].get('tipo_expediente')}")
        print(f"Sentido de la sentencia: {resultado['metadatas'][i].get('sentido_resolucion')}")
        print(f"Fragmento del texto (Primeros 150 caracteres): {resultado['documents'][i][:150]}...")
        
        # Mostramos los primeros 5 valores del vector generado por el nuevo modelo multilingüe
        vector = resultado['embeddings'][i]
        print(f"Vector (paraphrase-multilingual-MiniLM-L12-v2) - Dimensión {len(vector)}:")
        print(f"[{vector[0]:.4f}, {vector[1]:.4f}, {vector[2]:.4f}, {vector[3]:.4f}, ...]")
        print("-" * 40 + "\n")
